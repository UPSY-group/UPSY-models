...
contains
  ...
  subroutine remap_ice_geometry_model_sealevel( self, mesh_new, forcing, time)
    ...
    case ('fixed')
      self%SL( mesh_new%vi1:mesh_new%vi2) = C%fixed_sealevel
    case ('prescribed')
      call update_sealevel_in_model( forcing, mesh_new, time, self%SL)
    end select
