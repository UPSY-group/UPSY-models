module ice_velocity_model_data
  ...
  type, abstract, extends(atype_model) :: atype_ice_velocity_model_data
    ...
    real(dp), dimension(:  ), contiguous, pointer :: v_vav           => null()   ! [m yr^-1]   Vertically averaged ice velocity in the y-direction
    real(dp), dimension(:  ), contiguous, pointer :: u_vav_b         => null()   ! [m yr^-1]   Vertically averaged ice velocity in the x-direction on the b-grid
