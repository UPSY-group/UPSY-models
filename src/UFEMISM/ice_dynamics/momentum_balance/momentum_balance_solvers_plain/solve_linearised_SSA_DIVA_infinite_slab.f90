...
contains
    ...
    do row_tiuv = A_CSR%i1, A_CSR%i2
      ...
      elseif (self%mesh%TriBI( ti) > 0) then
        ...
        if (C%do_include_SSADIVA_crossterms) then
          ...
          call self%calc_SSA_DIVA_stiffness_matrix_row_free( u_ii_term, A_CSR, bb, row_tiuv)
        else
          ! Calculate matrix coefficients for the SSA sans the gradients of the effective viscosity (the "cross-terms")
          call self%calc_SSA_DIVA_sans_stiffness_matrix_row_free( u_ii_term, A_CSR, bb, row_tiuv)
        end if
  ...
  subroutine calc_SSA_DIVA_stiffness_matrix_row_free( self, u_ii_term, A_CSR, bb, row_tiuv)
    ...
    ! Local variables:
    integer                             :: ti, uv
    real(dp)                            :: N, dN_dx, dN_dy, u_ii_term_ii, tau_dx, tau_dy
    ...
    if (uv == 1) then
      ...
      do k = 1, single_row_nnz
        ...
        if (tj == ti) Au = Au - u_ii_term_ii                ! - beta_b u
    ...
    elseif (uv == 2) then
      ...
      do k = 1, single_row_nnz
        ...
        if (tj == ti) Av = Av - u_ii_term_ii                ! - beta_b v
  ...
  subroutine calc_SSA_DIVA_sans_stiffness_matrix_row_free( self, u_ii_term, A_CSR, bb, row_tiuv)
    !< Add coefficients to this matrix row to represent the linearised SSA
    ...
    ! Local variables:
    integer                             :: ti, uv
    real(dp)                            :: N, u_ii_term_ii, tau_dx, tau_dy
    integer,  dimension(:), allocatable :: single_row_ind
    real(dp), dimension(:), allocatable :: single_row_ddx_val
    real(dp), dimension(:), allocatable :: single_row_ddy_val
    real(dp), dimension(:), allocatable :: single_row_d2dx2_val
    ...
    if (uv == 1) then
      ...
      do k = 1, single_row_nnz
        ...
        if (tj == ti) Au = Au - u_ii_term_ii / N               ! - beta_b u / N
    ...
    elseif (uv == 2) then
      ...
      do k = 1, single_row_nnz
        ...
        if (tj == ti) Av = Av - u_ii_term_ii / N               ! - beta_b v / N
