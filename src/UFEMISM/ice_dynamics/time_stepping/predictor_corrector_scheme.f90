  ...
  subroutine run_ice_dynamics_model_pc( region, dt_max)
    ...
    iterate_pc_timestep: do while (pc_it < C%pc_nit_max)
      ...
      call calc_dHi_dt( region%mesh, region%ice, region%ice%Hi, region%ice%Hb, region%ice%SL, &
        region%ice%u_vav_b, region%ice%v_vav_b, SMB_loc, region%BMB%BMB, region%LMB%LMB, &
        region%AMB%AMB, region%ice%fraction_margin, region%ice%mask_noice, region%ice%pc%dt_np1, &
        region%ice%pc%dHi_dt_Hi_n_u_n, Hi_dummy, region%ice%divQ, region%ice%dHi_dt_target, region%name)
      ...
      call calc_dHi_dt( region%mesh, region%ice, region%ice%Hi, region%ice%Hb, region%ice%SL, &
       region%ice%u_vav_b, region%ice%v_vav_b, SMB_loc, region%BMB%BMB, region%LMB%LMB, &
       region%AMB%AMB, region%ice%fraction_margin, region%ice%mask_noice, region%ice%pc%dt_np1, &
       region%ice%pc%dHi_dt_Hi_star_np1_u_np1, Hi_dummy, region%ice%divQ, region%ice%dHi_dt_target, region%name)
