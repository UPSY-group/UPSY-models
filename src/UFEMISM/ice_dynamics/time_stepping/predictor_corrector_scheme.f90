...
contains
  ...
  subroutine run_ice_dynamics_model_pc( region, dt_max)
    ...
    ! type(type_ice_geometry)                              :: Pred_IG  !predicted ice geometry
    ...
    ! call Pred_IG%allocate_ice_geometry( region%mesh)
    ...
    iterate_pc_timestep: do while (pc_it < C%pc_nit_max)
      ...
      region%ice%pc%Hi_star_np1 = Prev_IG%Hi + region%ice%pc%dt_np1 * ((1._dp + region%ice%pc%zeta_t / 2._dp) * &
      ...
      ! call Pred_IG%calc_ice_geometry(region%mesh, region%ice%Hi_prev, region%ice%SL , region%ice%Hb)
    ...
    ! call calving_law(region%mesh, Corr_IG, region%ice%pc%Hi_np1, region%ice%dHi_dt)

    ! call kill_icebergs(region%mesh, Corr_IG)
    ...
    !     print *, 'Hi became negative in the PC scheme with a value of ', region%ice%Hi_next(vi), ' at vertex ', vi
    !   end if
    ! end do
