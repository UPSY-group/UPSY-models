...
contains
  ...
  subroutine run_ice_dynamics_model_pc( region, dt_max)
    ...
    do vi = region%mesh%vi1, region%mesh%vi2
      region%ice%Hi_prev               ( vi) = region%ice%Hi_next           ( vi)
      region%ice%pc%dHi_dt_Hi_nm1_u_nm1( vi) = region%ice%pc%dHi_dt_Hi_n_u_n( vi)
    ...
    iterate_pc_timestep: do while (pc_it < C%pc_nit_max)
      ...
      do vi = region%mesh%vi1, region%mesh%vi2
      region%ice%pc%Hi_np1( vi) = region%ice%Hi_prev( vi) + (region%ice%pc%dt_np1 / 2._dp) * &
        (region%ice%pc%dHi_dt_Hi_n_u_n( vi) + region%ice%pc%dHi_dt_Hi_star_np1_u_np1( vi))
      end do
    ...
    do vi = region%mesh%vi1, region%mesh%vi2
      region%ice%Hi_next( vi)   = region%ice%pc%Hi_np1( vi)
    end do
