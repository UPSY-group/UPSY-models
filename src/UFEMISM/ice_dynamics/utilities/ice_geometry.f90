module ice_geometry_calculations_mod
    ...
    subroutine allocate_ice_geometry (self, mesh)
        ...
        if (.not. allocated(self%bedrock_cdf)) allocate(self%bedrock_cdf(self%mesh%vi1:self%mesh%vi2, C%subgrid_bedrock_cdf_nbins))
        if (.not. allocated(self%bedrock_cdf_b)) allocate(self%bedrock_cdf_b(self%mesh%vi1:self%mesh%vi2, C%subgrid_bedrock_cdf_nbins))
