  ...
  subroutine write_to_main_regional_output_file_grid_field( region, grid, filename, ncid, choice_output_field)
    ...
    ! ======================
      ...
      case ('u_vav')
        ...
        case ('SSA_FEM_PETSc')
          ! These solvers define velocities on the a-grid (vertices)
          call map_from_mesh_vertices_to_xy_grid_2D( region%mesh, grid, C%output_dir, region%ice%vel%u_vav, d_grid_vec_partial_2D)
        end select
      ...
      case ('v_vav')
        ...
        case ('SSA_FEM_PETSc')
          ! These solvers define velocities on the a-grid (vertices)
          call map_from_mesh_vertices_to_xy_grid_2D( region%mesh, grid, C%output_dir, region%ice%vel%v_vav, d_grid_vec_partial_2D)
        end select
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'v_vav', d_grid_vec_partial_2D)
      case ('u_vav_a')
        call map_from_mesh_vertices_to_xy_grid_2D( region%mesh, grid, C%output_dir, region%ice%vel%u_vav, d_grid_vec_partial_2D)
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'u_vav_a', d_grid_vec_partial_2D)
      case ('v_vav_a')
        call map_from_mesh_vertices_to_xy_grid_2D( region%mesh, grid, C%output_dir, region%ice%vel%v_vav, d_grid_vec_partial_2D)
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'v_vav_a', d_grid_vec_partial_2D)
      case ('u_vav_b')
        call map_from_mesh_triangles_to_xy_grid_2D( region%mesh, grid, C%output_dir, region%ice%vel%u_vav_b, d_grid_vec_partial_2D)
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'u_vav_b', d_grid_vec_partial_2D)
      case ('v_vav_b')
        call map_from_mesh_triangles_to_xy_grid_2D( region%mesh, grid, C%output_dir, region%ice%vel%v_vav_b, d_grid_vec_partial_2D)
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'v_vav_b', d_grid_vec_partial_2D)
      case ('uabs_vav')
        select case (C%choice_stress_balance_approximation)
        case default
          call crash('invalid choice_stress_balance_approximation ' // trim( C%choice_stress_balance_approximation))
        ...
        case ('SSA_FEM_PETSc')
          ! These solvers define velocities on the a-grid (vertices)
          call map_from_mesh_vertices_to_xy_grid_2D( region%mesh, grid, C%output_dir, region%ice%vel%uabs_vav, d_grid_vec_partial_2D)
        end select
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'uabs_vav', d_grid_vec_partial_2D)
      case ('uabs_vav_a')
        call map_from_mesh_vertices_to_xy_grid_2D( region%mesh, grid, C%output_dir, region%ice%vel%uabs_vav, d_grid_vec_partial_2D)
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'uabs_vav_a', d_grid_vec_partial_2D)
      case ('uabs_vav_b')
        call map_from_mesh_triangles_to_xy_grid_2D( region%mesh, grid, C%output_dir, region%ice%vel%uabs_vav_b, d_grid_vec_partial_2D)
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'uabs_vav_b', d_grid_vec_partial_2D)
      ...
      case ('u_surf')
        ...
        case ('SSA_FEM_PETSc')
          ! These solvers define velocities on the a-grid (vertices)
          call map_from_mesh_vertices_to_xy_grid_2D( region%mesh, grid, C%output_dir, region%ice%vel%u_surf, d_grid_vec_partial_2D)
        end select
      ...
      case ('v_surf')
        ...
        case ('SSA_FEM_PETSc')
          ! These solvers define velocities on the a-grid (vertices)
          call map_from_mesh_vertices_to_xy_grid_2D( region%mesh, grid, C%output_dir, region%ice%vel%v_surf, d_grid_vec_partial_2D)
        end select
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'v_surf', d_grid_vec_partial_2D)
      case ('u_surf_a')
        call map_from_mesh_vertices_to_xy_grid_2D( region%mesh, grid, C%output_dir, region%ice%vel%u_surf, d_grid_vec_partial_2D)
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'u_surf_a', d_grid_vec_partial_2D)
      case ('v_surf_a')
        call map_from_mesh_vertices_to_xy_grid_2D( region%mesh, grid, C%output_dir, region%ice%vel%v_surf, d_grid_vec_partial_2D)
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'v_surf_a', d_grid_vec_partial_2D)
      case ('u_surf_b')
        call map_from_mesh_triangles_to_xy_grid_2D( region%mesh, grid, C%output_dir, region%ice%vel%u_surf_b, d_grid_vec_partial_2D)
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'u_surf_b', d_grid_vec_partial_2D)
      case ('v_surf_b')
        call map_from_mesh_triangles_to_xy_grid_2D( region%mesh, grid, C%output_dir, region%ice%vel%v_surf_b, d_grid_vec_partial_2D)
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'v_surf_b', d_grid_vec_partial_2D)
      case ('uabs_surf')
        select case (C%choice_stress_balance_approximation)
        case default
          call crash('invalid choice_stress_balance_approximation ' // trim( C%choice_stress_balance_approximation))
        ...
        case ('SSA_FEM_PETSc')
          ! These solvers define velocities on the a-grid (vertices)
          call map_from_mesh_vertices_to_xy_grid_2D( region%mesh, grid, C%output_dir, region%ice%vel%uabs_surf, d_grid_vec_partial_2D)
        end select
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'uabs_surf', d_grid_vec_partial_2D)
      case ('uabs_surf_a')
        call map_from_mesh_vertices_to_xy_grid_2D( region%mesh, grid, C%output_dir, region%ice%vel%uabs_surf, d_grid_vec_partial_2D)
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'uabs_surf_a', d_grid_vec_partial_2D)
      case ('uabs_surf_b')
        call map_from_mesh_triangles_to_xy_grid_2D( region%mesh, grid, C%output_dir, region%ice%vel%uabs_surf_b, d_grid_vec_partial_2D)
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'uabs_surf_b', d_grid_vec_partial_2D)
      case ('w_surf')
  ...
  subroutine create_main_regional_output_file_grid_field( filename, ncid, choice_output_field)
    ...
    ! ======================
      ...
      case ('u_vav')
        ...
        case ('SSA_FEM_PETSc')
          ! These solvers define velocities on the a-grid (vertices)
          call add_field_grid_dp_2D( filename, ncid, 'u_vav', precision = C%output_precision, do_compress = C%do_compress_output, long_name = 'Vertically averaged ice velocity in the x-direction on the mesh vertices', units = 'm yr^-1')
        end select
      case ('v_vav')
        select case (C%choice_stress_balance_approximation)
        case default
        ...
        case ('SSA_FEM_PETSc')
          ! These solvers define velocities on the a-grid (vertices)
          call add_field_grid_dp_2D( filename, ncid, 'v_vav', precision = C%output_precision, do_compress = C%do_compress_output, long_name = 'Vertically averaged ice velocity in the y-direction on the mesh vertices', units = 'm yr^-1')
        end select
      case ('u_vav_a')
        call add_field_grid_dp_2D( filename, ncid, 'u_vav_a', precision = C%output_precision, do_compress = C%do_compress_output, long_name = 'Vertically averaged ice velocity in the x-direction on the mesh vertices', units = 'm yr^-1')
      case ('v_vav_a')
        call add_field_grid_dp_2D( filename, ncid, 'v_vav_a', precision = C%output_precision, do_compress = C%do_compress_output, long_name = 'Vertically averaged ice velocity in the y-direction on the mesh vertices', units = 'm yr^-1')
      case ('u_vav_b')
        call add_field_grid_dp_2D( filename, ncid, 'u_vav_b', precision = C%output_precision, do_compress = C%do_compress_output, long_name = 'Vertically averaged ice velocity in the x-direction on the mesh triangles', units = 'm yr^-1')
      case ('v_vav_b')
        call add_field_grid_dp_2D( filename, ncid, 'v_vav_b', precision = C%output_precision, do_compress = C%do_compress_output, long_name = 'Vertically averaged ice velocity in the y-direction on the mesh triangles', units = 'm yr^-1')
      case ('uabs_vav')
        select case (C%choice_stress_balance_approximation)
        case default
          call crash('invalid choice_stress_balance_approximation ' // trim( C%choice_stress_balance_approximation))
        ...
        case ('SSA_FEM_PETSc')
          ! These solvers define velocities on the a-grid (vertices)
          call add_field_grid_dp_2D( filename, ncid, 'uabs_vav', precision = C%output_precision, do_compress = C%do_compress_output, long_name = 'Vertically averaged ice speed on the mesh vertices', units = 'm yr^-1')
        end select
      case ('uabs_vav_a')
        call add_field_grid_dp_2D( filename, ncid, 'uabs_vav_a', precision = C%output_precision, do_compress = C%do_compress_output, long_name = 'Vertically averaged ice speed on the mesh vertices', units = 'm yr^-1')
      case ('uabs_vav_b')
        call add_field_grid_dp_2D( filename, ncid, 'uabs_vav_b', precision = C%output_precision, do_compress = C%do_compress_output, long_name = 'Vertically averaged ice speed on the mesh triangles', units = 'm yr^-1')
      ...
      case ('u_surf')
        ...
        case ('SSA_FEM_PETSc')
          ! These solvers define velocities on the a-grid (vertices)
          call add_field_grid_dp_2D( filename, ncid, 'u_surf', precision = C%output_precision, do_compress = C%do_compress_output, long_name = 'Surface ice velocity in the x-direction on the mesh vertices', units = 'm yr^-1')
        end select
      case ('v_surf')
        select case (C%choice_stress_balance_approximation)
        case default
        ...
        case ('SSA_FEM_PETSc')
          ! These solvers define velocities on the a-grid (vertices)
          call add_field_grid_dp_2D( filename, ncid, 'v_surf', precision = C%output_precision, do_compress = C%do_compress_output, long_name = 'Surface ice velocity in the y-direction on the mesh vertices', units = 'm yr^-1')
        end select
      case ('u_surf_a')
        call add_field_grid_dp_2D( filename, ncid, 'u_surf_a', precision = C%output_precision, do_compress = C%do_compress_output, long_name = 'Surface ice velocity in the x-direction on the mesh vertices', units = 'm yr^-1')
      case ('v_surf_a')
        call add_field_grid_dp_2D( filename, ncid, 'v_surf_a', precision = C%output_precision, do_compress = C%do_compress_output, long_name = 'Surface ice velocity in the y-direction on the mesh vertices', units = 'm yr^-1')
      case ('u_surf_b')
        call add_field_grid_dp_2D( filename, ncid, 'u_surf_b', precision = C%output_precision, do_compress = C%do_compress_output, long_name = 'Surface ice velocity in the x-direction on the mesh triangles', units = 'm yr^-1')
      case ('v_surf_b')
        call add_field_grid_dp_2D( filename, ncid, 'v_surf_b', precision = C%output_precision, do_compress = C%do_compress_output, long_name = 'Surface ice velocity in the y-direction on the mesh triangles', units = 'm yr^-1')
      case ('uabs_surf')
        select case (C%choice_stress_balance_approximation)
        case default
          call crash('invalid choice_stress_balance_approximation ' // trim( C%choice_stress_balance_approximation))
        ...
        case ('SSA_FEM_PETSc')
          ! These solvers define velocities on the a-grid (vertices)
          call add_field_grid_dp_2D( filename, ncid, 'uabs_surf', precision = C%output_precision, do_compress = C%do_compress_output, long_name = 'Surface ice speed on the mesh vertices', units = 'm yr^-1')
        end select
      case ('uabs_surf_a')
        call add_field_grid_dp_2D( filename, ncid, 'uabs_surf_a', precision = C%output_precision, do_compress = C%do_compress_output, long_name = 'Surface ice speed on the mesh vertices', units = 'm yr^-1')
      case ('uabs_surf_b')
        call add_field_grid_dp_2D( filename, ncid, 'uabs_surf_b', precision = C%output_precision, do_compress = C%do_compress_output, long_name = 'Surface ice speed on the mesh triangles', units = 'm yr^-1')
      case ('w_surf')
