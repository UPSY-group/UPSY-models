...
contains
  ...
  subroutine test_demo_model( test_name_parent, mesh1, mesh2)
    ...
    call demo_model1%allocate  ( demo_model1%ct_allocate( mesh1))
    call demo_model1%initialise( demo_model1%ct_initialise( 1, 2))

    ! Run the demo model
    call demo_model1%run( demo_model1%ct_run( 3, 4))
    ...
    call demo_model2%allocate( demo_model2%ct_allocate( mesh1))
    ...
    call demo_model1%remap( demo_model1%ct_remap( mesh2))
    call demo_model1%initialise( demo_model1%ct_initialise( 1, 2))

    ! Create and initialise a third instance of the demo model directly on the new mesh
    call demo_model3%allocate  ( demo_model3%ct_allocate( mesh1))
    call demo_model3%initialise( demo_model3%ct_initialise( 1, 2))
